...
Below is a code block that will be filled with [hello-world.go](./assets/helloworld.go) contents on running timd. We use the standard go-present notation using sed-like search terms to select which part of the file we wish to show `<!-- .code file.go /start/,/end/ -->`. All lines that have the `omit` keyword are omitted. `omit` must be separated by spaces or a newline.
